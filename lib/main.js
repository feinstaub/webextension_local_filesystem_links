...
function main() {
    mod = pageMod.PageMod( {
        include: prefs.options.whitelist,//.split( /\s+/ ), // || "*",
        ...
        onAttach: function( worker ) {
            ...
            console.log( "onAttach", mod.include );
    ...
    function onWhitelistApply( ) {
        console.log( "whitelist applied", prefs.options[ "whitelist" ] );

        //Console.log( mod.include );

        refreshPageMod( prefs.options[ "whitelist" ] );

        //Mod.include = simplePrefs.prefs[ prefName ];
    }

    prefs.addPrefChangeHandler( "applyWhitelist", onWhitelistApply );

    function refreshPageMod( whitelist ) {
        console.log( "refresh pagemod", whitelist );

        mod.destroy();

        // Reloading all tabs not working
        // --> after mod.destroy() there will be unstyled content of the icon in the page
        // --> fix later
        //
        // For ( var tab of tabs ) {
        //     tab.reload;
