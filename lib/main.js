...
exports.main = function(options, callbacks) {
  ...
  const menuItem3 = contextMenu.Item({
    label: "Open with Windows Explorer", // TODO: make this dependent on OS
    context: contextMenu.SelectionContext(),
  ...
  hyperlinkScanner = pageMod.PageMod({
    include: ['none://*'], // see applySettings
...
//
// WARN: call only once because we are giving a ref to main-widget
...
//    };
    // TODO: ok?
    settings.enableScanning = ss.storage.settings.enableScanning;
    settings.scannerIncludePatternArray = ss.storage.settings.scannerIncludePatternArray;
  }
  
  //// console.log("total: " + JSON.stringify(settings));
